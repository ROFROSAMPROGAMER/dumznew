...
module.exports = async (client, message) => {
    ...
    if (cmd.length === 0){
      ...
      if(matchedPrefix.includes(client.user.id)) {
        return message.channel.send(new MessageEmbed().setTitle(`Huh? I was Pinged?`).setDescription(`**Start Listening to Music with: \`${prefix}play [SONG]\`, to see all Commands type: \`${prefix}help\`**`));
      }
      return;
      }
    ...
    if (command){
        ...
        if (timestamps.has(message.author.id)) { //if the user is on cooldown
          ...
          if (now < expirationTime) { //if he is still on cooldonw
            ...
            not_allowed = true;
            return message.channel.send(`<:no:878698997152374814> Please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${command.name}\` command.`); //send an information message
          }
        }
        ...
        if(command.memberpermissions) {
          if (!message.member.hasPermission(command.memberpermissions)) {
            ...
            message.channel.send(new Discord.MessageEmbed()
              ...
              .setTitle("<:no:878698997152374814> You are not allowed to run this command!")
          ...
          if(command.parameters.type == "music"){
            ...
            if (!channel) {
              ...
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:878698997152374814> You need to join a voice channel.`)
              );
            }
